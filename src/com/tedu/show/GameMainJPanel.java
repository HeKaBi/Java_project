...
public class GameMainJPanel extends JPanel implements Runnable {
    ...
    private void drawWeaponIndicator(Graphics2D g2, int centerX, int baselineY,
                                     String label, boolean selected, boolean enabled) {
        Font displayFont = resolveDisplayFont(HUD_SMALL_FONT, label);
        g2.setFont(displayFont);
        FontMetrics metrics = g2.getFontMetrics(displayFont);
        int w = Math.max(18, metrics.stringWidth(label) + 10);
        int h = 14;
        int x = centerX - w / 2;
        int y = baselineY - h;
        g2.setColor(enabled ? (selected ? HUD_SLOT_ACTIVE : HUD_SLOT_FILL) : HUD_SLOT_DISABLED);
        ...
        g2.setColor(enabled ? (selected ? HUD_GOLD : HUD_BORDER) : new Color(108, 101, 85));
        g2.drawRoundRect(x, y, w, h, 6, 6);
        drawCenteredOutlinedText(g2, label, x, y, w, h, displayFont,
                selected ? HUD_GOLD : (enabled ? HUD_CYAN : new Color(138, 134, 122)), Color.BLACK);
    }
